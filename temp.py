...
# Dozenary tree
def solve_tree(upper,down,front,back,left,right,):
    # Set Queue and Solve Path
    queue = deque([((upper,down,front,back,left,right),"")])
    ...
    while queue:
        # Queue
        (c_upper,c_down,c_front,c_back,c_left,c_right), path = queue.popleft()

        # If depth limit reached, stop
        if(len(path) > 4*max_depth):
            print("Reached Depth Limit" + ". Stopping.")
            break
        ...
        next_states.append(((c_upper[:],c_down[:],c_front[:], c_back[:], c_left[:],c_right[:]), path + "U ->"))
        ...
        next_states.append(((c_upper[:],c_down[:],c_front[:], c_back[:], c_left[:],c_right[:]), path + "F ->"))
        ...
        next_states.append(((c_upper[:],c_down[:],c_front[:], c_back[:], c_left[:],c_right[:]), path + "L ->"))
...
def solve(upper,down,front,back,left,right):
    ...
    solve_path = ""
    solve_path = solve_tree(upper,down,front,back,left,right)
    out_stack = str2stack(solve_path)

    print("Solve Steps: " + str(solve_path))
    print("--- %s seconds ---" % (time.time() - start_time))
    print("Steps: " + str(len(solve_path)//4))
    return(solve_path)
